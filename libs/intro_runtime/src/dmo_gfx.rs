...
impl DmoGfx {
    pub fn new_with_dimensions(
        ...
        DmoGfx {
            settings: Settings::default(),
            context: ContextGfx::new_with_dimensions(
                window_width,
                window_height,
                screen_width,
                screen_height,
    ...
    pub fn compile_model_shaders(
        &mut self,
        model_idx: usize,
        err_msg_buf: &mut [u8; ERR_MSG_LEN],
    ) -> Result<(), RuntimeError> {
        for mesh in self.context.polygon_context.models[model_idx]
            .meshes
            .iter_mut()
        {
    ...
    pub fn update_polygon_context_projection(&mut self, aspect: f32) {
        self.context
            .polygon_context
            .update_projection_matrix(aspect);
    }
    ...
    pub fn update_polygon_context(&mut self) -> Result<(), RuntimeError> {
        ...
        self.context.polygon_context.view_position = Vector3::new(
            self.context.sync_vars.get_builtin(Camera_Pos_X) as f32,
            self.context.sync_vars.get_builtin(Camera_Pos_Y) as f32,
            self.context.sync_vars.get_builtin(Camera_Pos_Z) as f32,
        );

        self.context.polygon_context.view_front = Vector3::new(
            self.context.sync_vars.get_builtin(Camera_Front_X) as f32,
            ...
            for mut scene_object in scene.scene_objects.iter_mut() {
                match scene_object.position_var {
                    ...
                    ValueVec3::Sync(x, y, z) => {
                        scene_object.position = Vector3::new(
                            self.context.sync_vars.get_index(x as usize)? as f32,
                ...
                match scene_object.euler_rotation_var {
                    ValueVec3::NOOP => {}
                    ValueVec3::Sync(x, y, z) => {
                        scene_object.euler_rotation = Vector3::new(
                            to_radians(self.context.sync_vars.get_index(x as usize)? as f32),
                    ...
                    ValueVec3::Fixed(x, y, z) => {
                        scene_object.euler_rotation =
                            Vector3::new(to_radians(x), to_radians(y), to_radians(z));
                    }
                }
