...
impl JitFn {
    pub fn new(num_pages: usize, context: &mut Context, operators: &SmallVec<[Op; 64]>) -> JitFn {
...
pub trait JitAssembler {
    ...
    fn fill_jit(&mut self, context: &mut Context, operators: &SmallVec<[Op; 64]>);
...
impl JitAssembler for JitMemory {
    ...
    fn to_jit_fn(&mut self) -> JitFn {
        unsafe {
            ...
            clear_cache(self.addr as *mut _, (self.addr as *mut i8).offset(self.size as _));
        }
    ...
    fn fill_jit(&mut self, context: &mut Context, operators: &SmallVec<[Op; 64]>) {
        ...
        for op in operators.iter() {
            match *op {
                ...
                Op::Draw_Quad_Scene(scene_idx) => {
                    // rdi: pointer to Context (pointer is an integer value)
                    self.movabs_rdi_u64( unsafe { mem::transmute(context as *mut _) });
                    // rsi: scene_id (interger)
                    self.movabs_rsi_u64(scene_idx as u64);

                    self.movabs_rax_u64( unsafe { mem::transmute(
                        Ops::op_draw_quad_scene as extern "sysv64" fn(&Context, u8)
                    )});
                    self.call_rax();
                },


                Op::If_Var_Equal_Draw_Quad(var_idx, value, scene_idx) => {
                    self.movabs_rdi_u64( unsafe { mem::transmute(context as *mut _) });
                    self.movabs_rsi_u64(var_idx as u64);
                    self.movss_xmm_n_f64(0, value);
                    self.movabs_rdx_u64(scene_idx as u64);

                    self.movabs_rax_u64( unsafe { mem::transmute(
                        Ops::op_if_var_equal_draw_quad as extern "sysv64" fn(&Context, u8, f64, u8)
                    )});
                    self.call_rax();
                },

                Op::If_Var_Equal_Draw_Polygon(var_idx, value, scene_idx) => {
                    self.movabs_rdi_u64( unsafe { mem::transmute(context as *mut _) });
                    self.movabs_rsi_u64(var_idx as u64);
                    self.movss_xmm_n_f64(0, value);
                    self.movabs_rdx_u64(scene_idx as u64);
                    ...
                    self.movabs_rax_u64( unsafe { mem::transmute(
                        Ops::op_if_var_equal_draw_polygon as extern "sysv64" fn(&Context, u8, f64, u8)
                    )});
                    self.call_rax();
                },

                Op::Clear(red, green, blue, alpha) => {
                    // rdi: pointer to Context
                    self.movabs_rdi_u64( unsafe { mem::transmute(context as *mut _) });
                    // red, green, blue, alpha
                    self.movabs_rsi_u64(red as u64);
                    self.movabs_rdx_u64(green as u64);
                    self.movabs_rcx_u64(blue as u64);
                    self.movabs_r8_u64(alpha as u64);

                    self.movabs_rax_u64( unsafe { mem::transmute(
                        Ops::op_clear as extern "sysv64" fn(&Context, u8, u8, u8, u8)
                    )});
                    self.call_rax();
                },

                Op::Target_Buffer(idx) => {
                    self.movabs_rdi_u64( unsafe { mem::transmute(context as *mut _) });
                    self.movabs_rsi_u64(idx as u64);
                    self.movabs_rax_u64( unsafe { mem::transmute(
                        Ops::op_target_buffer as extern "sysv64" fn(&Context, u8)
                    )});
                    self.call_rax();
                },

                Op::Target_Buffer_Default => {
                    self.movabs_rdi_u64( unsafe { mem::transmute(context as *mut _) });
                    self.movabs_rax_u64( unsafe { mem::transmute(
                        Ops::op_target_buffer_default as extern "sysv64" fn(&Context)
                    )});
                    self.call_rax();
                },

                Op::Profile_Event(idx) => {
                    self.movabs_rdi_u64( unsafe { mem::transmute(context as *mut _) });
                    self.movabs_rsi_u64(idx as u64);
                    self.movabs_rax_u64( unsafe { mem::transmute(
                        Ops::op_profile_event as extern "sysv64" fn(&mut Context, u8)
                    )});
                    self.call_rax();
                },
