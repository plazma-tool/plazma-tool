...
pub fn compile_shader(src: &str, ty: GLenum, err_msg_buf: &mut [u8; ERR_MSG_LEN]) -> Result<GLuint, RuntimeError> {
    let shader;
    unsafe {
...
pub fn link_program(vs: GLuint, fs: GLuint, err_msg_buf: &mut [u8; ERR_MSG_LEN]) -> Result<GLuint, RuntimeError> {
    let program;
    unsafe {
